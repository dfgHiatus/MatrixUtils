...
//					m1[i,j]=new Fraction(Console.ReadLine());
			...
			try
			{
				det=m1.DeterminentFast();
			}
			catch(Exception)
			{
				Console.WriteLine("DeterminentFast() method was failed due to overflow, now calculating determinent "
				                  + "using traditional minor method, this may take more time...");
				det=m1.Determinent();	
			}
			try
			{
				inverse=m1.InverseFast();
			}
			catch(Exception)
			{
				Console.WriteLine("InverseFast() method was failed due to overflow, now calculating inverse "
                 				+ "using traditional adjoint method, this may take more time...");
				inverse=m1.Inverse();	
			}
		...
		public static void EquationSolver()
		{
				...
				for (int k=0;k<i;k++)
				{
					...
					for (int nn=0;nn<iVarNo+1;nn++)
					{
						if (nn>0 && nn<iVarNo)
							Console.Write(" + ");
						if (nn<iVarNo)
							Console.Write("X"+(nn+1)+"*("+mat[k,nn]+")");
						else 
							Console.Write(" = "+mat[k,nn]  );
					}	
				}
				Console.Write("\n\n");
			...
			for (i=0;i<iVarNo;i++)
					Console.Write("X"+(i+1)+"="+ech[i,iVarNo]+"\t");
		}	//end EquationSolver()
